...
const formSchema = z.object({
  ...
  purchasePrice: z
    .string()
    .refine((val) => !isNaN(parseFloat(val)), { message: 'Preço de compra inválido.' }),
  salePrice: z
    .string()
    .refine((val) => !isNaN(parseFloat(val)), { message: 'Preço de venda inválido.' }),
  description: z.string().optional(),
});

type ProdutoFormData = z.infer<typeof formSchema>;

type EditProdutoFormProps = {
  productId: string;
};
      ...
      if (result.suggestedPrice) {
        ...
        toast({
          ...
          description: (
            <div>
              <p>
                Preço de venda atualizado para{' '}
                {result.suggestedPrice.toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL',
                })}
                .
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Ex: Azul, Verde" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
